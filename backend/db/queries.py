...
from backend.models.orm import AudioTrack, Library, Prompt, Session, User
...
class AudioTrackQueries:
    ...
    async def get_by_name(db: AsyncSession, name: str) -> Optional[AudioTrack]:
        result = await db.execute(select(AudioTrack).where(AudioTrack.name == name))
        return result.scalar_one_or_none()

    @staticmethod
    async def list_tracks(
        db: AsyncSession, skip: int = 0, limit: int = 100, audio_type: Optional[str] = None
    ) -> List[AudioTrack]:
    ...
    async def list_by_audio_type(
        db: AsyncSession, audio_type: str, skip: int = 0, limit: int = 100
    ) -> List[AudioTrack]:
        result = await db.execute(
            select(AudioTrack).where(AudioTrack.audio_type == audio_type).offset(skip).limit(limit)
        )
        return result.scalars().all()
...
class PromptQueries:
    ...
    async def get_active_by_name(db: AsyncSession, name: str) -> Optional[Prompt]:
        result = await db.execute(select(Prompt).where(Prompt.name == name, Prompt.is_active))
        return result.scalar_one_or_none()

    @staticmethod
    async def list_active(db: AsyncSession) -> List[Prompt]:
        result = await db.execute(select(Prompt).where(Prompt.is_active))
        return result.scalars().all()
